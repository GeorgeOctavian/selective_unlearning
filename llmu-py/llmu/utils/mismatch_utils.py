...
def create_forget_dataloader_from_Pile(tokenizer, dataset, batch_size=4):
    ...
    def preproccess(examples):
        ...
        for i in range(len(examples["text"])):
            ...
            # all have 200 length
            results["input_ids"].append(source["input_ids"].squeeze())
            results["attention_mask"].append(source["attention_mask"].squeeze())
            results["start_locs"].append(50)
...
def get_answer_loss(tokenizer, operation, batch, model, device="cuda:0"):
    """
...
def get_rand_complete_loss(bad_batch, tokenizer, normal_ans, model, K=5, device="cuda:0"):
    ...
    for batch_idx in range(bad_input_ids.shape[0]):
        ...
        prefix_input_id = single_input_id[:50]
        prefix_text = tokenizer.decode(prefix_input_id)

        # Get random answer.
        for rand_ans in rand_ans_list:
            random_sample = f"{prefix_text} {rand_ans}"

            # Tokenize.
            tokenized_rs = tokenizer(
                random_sample, truncation=True, padding="max_length", max_length=200
            )
    ...
    # GD on answer.
    random_loss = get_answer_loss(tokenizer, "gd", batch_random, model, device=device)
